...
def get_head_contour(image, n_points=500):
    ...
    mask = MediaPipeUtilities.segment_head(image)
...
def warp_images(img1, img2, pts1, pts2, pts_mid):
    ...
    # plt.imshow(np.ones_like(img1)*255)  # biele pozadie
    # plt.triplot(pts_mid[:,0], pts_mid[:,1], tri.simplices, color='black')
    # plt.plot(pts_mid[:,0], pts_mid[:,1], 'o', color='red', markersize=1)
    # plt.title("Delaunay triangulácia na hlave")
    # plt.axis('off')
    # plt.savefig("obr_delaunay_head.png", dpi=300, bbox_inches='tight')
    # plt.show()
...
def morph_head(img1, img2, alpha=0.5, margin=50, axis="x"):
    ...
    head1 = MediaPipeUtilities.extract_head_only(img1_fg)
    plt.imshow(head1)
    plt.scatter(pts1[:, 0], pts1[:, 1], s=3, c='yellow', alpha=0.5)
    plt.show()
    head2 = MediaPipeUtilities.extract_head_only(img2_fg)
    plt.imshow(head2)
    plt.scatter(pts2[:, 0], pts2[:, 1], s=3, c='yellow', alpha=0.5)
    plt.show()
...
if __name__ == "__main__":
    ...
    # # fg, bg_mask = MediaPipeUtilities.remove_background(res, thresh=0.5)
    # # plt.imshow(fg)
    # # plt.show()
    # res = MediaPipeUtilities.extract_head_only(res)
    # # res = MediaPipeUtilities.crop_head_rgba(res)
    ...
    metrics = MediaPipeUtilities.compare_generated_to_gt(res, gt)
    print(metrics)
    # res = MediaPipeUtilities.crop_head_rgba(res)
    plt.figure(figsize=(6,6))
    plt.imshow(cv2.cvtColor(res, cv2.COLOR_BGR2RGB))
