...
# IMAGE_PATH_A = 'dataset/dataset/BL.jpeg' # ! NASTAV CESTU K LAVEMU OBRAZKU
# IMAGE_PATH_B = 'dataset/dataset/BR.jpeg' # ! NASTAV CESTU K PRAVEMU OBRAZKU
...
if __name__ == "__main__":
    ...
    if not final_matched_A: # Skontroluj, či máme vôbec nejaké body
        ...
        print("Warpujem obrázok A na stred...")
        # Použijeme all_points_A ako zdroj, mid_points ako cieľ, triangles_indices ako definíciu trojuholníkov
        warped_A_advanced, warped_mask_A = faceWarp.warp_image_piecewise_affine(
        ...
        # morphed_image = post_process_face(morphed_image, blended_mask)

        end_morph_time = time.time()
        print(f"Morfovanie dokončené za {end_morph_time - start_morph_time:.2f}s.")

        # if warped_A_advanced.shape[2] == 3: # Ak je to farebný obrázok
        #     display_warped_A = cv2.cvtColor(warped_A_advanced, cv2.COLOR_BGR2RGB)
        #     display_warped_B = cv2.cvtColor(warped_B_advanced, cv2.COLOR_BGR2RGB)
        #     display_morphed_image = cv2.cvtColor(morphed_image, cv2.COLOR_BGR2RGB)
        # else: # Ak by to boli náhodou grayscale
        #     display_warped_A = warped_A_advanced
        #     display_warped_B = warped_B_advanced
        #     display_morphed_image = morphed_image


        # plt.imshow(warped_A_advanced) # Aktualizované zobrazenie pre warped_A
        # plt.title("Warped A (Advanced)")
        # plt.show()
        # plt.imshow(warped_B_advanced) # Aktualizované zobrazenie pre warped_B
        # plt.title("Warped B (Advanced)")
        # plt.show()
        # plt.imshow(morphed_image) # Finálny výsledok
        # plt.title("Morphed Image (Advanced)")
        # plt.show()
        ...
        if SHOW_PLOTS:
            ...
            points_A_middle = all_points_A * 0.5
            points_B_middle = all_points_B * 0.5
